...
class AnonCredsPresExchangeHandler(V20PresFormatHandler):
    ...
    async def verify_pres(self, pres_ex_record: V20PresExRecord) -> V20PresExRecord:
        ...
        #post-campaign diagnostic: time the Agent's own resolution so the enclave build's
        #native re-verification can be decomposed the same way the baseline's is
        ...
        print("[timing] Agent re-verify: " + " | ".join(spans), flush=True)
