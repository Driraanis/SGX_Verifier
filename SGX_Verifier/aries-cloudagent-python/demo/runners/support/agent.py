...
AGENT_SPANS_MARKER = "[timing] Agent re-verify: "


def split_agent_spans(output: tuple, source: str) -> list:
    """Split a combined agent timing line into one line per span.
    ...
    if source != "stdout" or len(output) != 1 or not isinstance(output[0], str):
        return []
    if AGENT_SPANS_MARKER not in output[0]:
        return []
    spans = output[0].partition(AGENT_SPANS_MARKER)[2].strip().split(" | ")
    return [f"{AGENT_SPANS_MARKER}{span}" for span in spans]
...
class DemoAgent:
    ...
    def handle_output(self, *output, source: str = None, **kwargs):
        ...
        spans = split_agent_spans(output, source)
        if spans:
            for span in spans:
