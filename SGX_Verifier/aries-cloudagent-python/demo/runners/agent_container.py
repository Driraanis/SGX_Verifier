...
#REE-side bridge to the in-enclave verifier. only the verifier role uses it
#(gated by self.use_enclave_verification); other agents keep ACA-Py's native
#path, so importing it here doesn't affect them. absolute form (not
#`from .enclave_client`): the demo runs as `python3 -m runners.x`, so `runners`
#is the package root and the relative form wouldn't resolve
from runners.enclave_client import verify_in_enclave  # noqa:E402

#colour for the timer we print below. log_msg carries it through prompt_toolkit,
#which escapes a raw ANSI code while the menu prompt is up
from runners.vlog import VIOLET_STYLE  # noqa:E402
...
class AriesAgent(DemoAgent):
    def __init__(
        ...
        **kwargs,
        ...
        #default off: the base agent and every non-verifier runner (alice, faber,
        #acme) use ACA-Py's native path. only VerifierAgent flips this to True to
        #route verification into the enclave
        self.use_enclave_verification = False
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state in ["request-received"]:
            ...
            log_status("#27 Process the proof provided by X")
            log_status("#28 Check if proof is valid")
            if self.use_enclave_verification:
                #verifier: the trusted verdict comes from inside the enclave
                proof = await self._verify_in_enclave(pres_ex_id, message)
            else:
                #every other agent: ACA-Py's native verification
                proof = await self.admin_POST(
    ...
    async def _verify_in_enclave(self, pres_ex_id: str, message: dict) -> dict:
        """Verify a received presentation inside the enclave (verifier role).

        Pulls the presentation and proof request from the exchange record's
        by_format, sends them to the enclave for the authoritative verdict, then
        still calls ACA-Py's native verify-presentation purely to drive the holder
        ack (ACA-Py couples the ack to that call). The returned dict is the
        enclave's verdict, kept in the shape the native path produced so the
        downstream log/store is unchanged. A genuine "no" (verified=False) is kept
        distinct from a "could not evaluate" (verified=None + error), so a
        server-side failure is never reported as an invalid proof.

        Args: pres_ex_id, message (the presentation-received webhook payload).
        Returns: the enclave verdict dict ({"verified": bool|None, ...}).
        """
        #by_format may already be on the webhook; if not, fetch the full record
        by_format = message.get("by_format")
        if not by_format:
            record = await self.admin_GET(
                f"/present-proof-2.0/records/{pres_ex_id}"
            )
            by_format = record.get("by_format", {})

        #pull the presentation and proof request, each keyed by format. anoncreds
        #first because this deployment runs the askar-anoncreds wallet — indy-first
        #would grab the wrong shape if both keys were present
        pres_by_format = by_format.get("pres", {})
        pres_req_by_format = by_format.get("pres_request", {})
        pres = pres_by_format.get("anoncreds") or pres_by_format.get("indy")
        pres_req = pres_req_by_format.get("anoncreds") or pres_req_by_format.get(
            "indy"
        )

        #ask the enclave for the trusted verdict. arg order (pres_req, pres) mirrors
        #the enclave's verify() and must not be swapped (both are dicts)
        try:
            verified, msgs = await verify_in_enclave(pres_req, pres)
            #a real verdict (true or false): report it as the verifier's answer
            proof = {"verified": "true" if verified else "false", "msgs": msgs}
        except RuntimeError as err:
            #couldn't evaluate (enclave unreachable / non-200): not a "no"
            self.log(f"Enclave verification could not evaluate: {err}")
            proof = {"verified": None, "error": str(err)}

        #always fire ACA-Py's native verification next, only so the protocol sends
        #the ack and the holder's exchange completes — including after an enclave
        #error above. a couldn't-evaluate is a verifier-side failure (enclave
        #unreachable), not a bad proof, so the holder mustn't hang in
        #presentation-sent for our hiccup. this call drives ONLY the ack; its
        #verdict is never used — `proof` already holds the enclave's verdict (or
        #verified=None) and is returned unchanged. guarded so an ack hiccup can't
        #mask the enclave result
        try:
            #this span bundles three things that can't be split without editing ACA-Py
            #core: the redundant AnonCreds verification, the record save, and the ack.
            #the ack is one loopback DIDComm message, so the number is dominated by the
            ...
            log_msg(
                f"[timing] Agent re-verify (verification + ack): {time.perf_counter() - native_t0:.6f} s",
                color=VIOLET_STYLE,
            )
        except Exception as err:
            self.log(f"Native ack-verify failed (enclave verdict stands): {err}")
