...
class AnonCredsVerifier:
    ...
    async def verify_presentation(
        ...
        """
        msgs = []
        #time the same phases the enclave brackets, so the two verifications can be
        #compared phase by phase. timer reads only: no logic, control flow or verdict
        #changes. the spans are stored rather than printed here so the caller can emit
        #all three native timers as one adjacent block
        self.prevalidation_s = None
        self.crypto_s = None
        ...
        try:
            ...
            #the failing path ends here, so close the span before returning
            self.prevalidation_s = perf_counter() - prevalidation_t0
            return (False, msgs)
        self.prevalidation_s = perf_counter() - prevalidation_t0
        ...
        try:
            ...
            verified = False
        #both branches land here, so one assignment covers the failing path too
        self.crypto_s = perf_counter() - crypto_t0
