...
class AnonCredsPresExchangeHandler(V20PresFormatHandler):
    ...
    async def verify_pres(self, pres_ex_record: V20PresExRecord) -> V20PresExRecord:
        ...
        #the enclave resolves these itself and times it as "ledger fetch"; here the
        #Agent resolves them, so bracketing the same call makes the two comparable.
        #timer read only, no behaviour change
        ...
        ledger_s = perf_counter() - ledger_t0
        ...
        #one line, not three: the runner relays this process's stdout a line at a time and
        #can service a webhook between two reads, which splits consecutive writes apart.
        #a single write is relayed by a single log_msg, so it cannot be interleaved
        spans = [f"ledger fetch {ledger_s:.6f} s"]
        if verifier.prevalidation_s is not None:
            spans.append(f"pre-validation {verifier.prevalidation_s:.6f} s")
        if verifier.crypto_s is not None:
            spans.append(f"crypto verification {verifier.crypto_s:.6f} s")
        print("[timing] native verification: " + " | ".join(spans), flush=True)
