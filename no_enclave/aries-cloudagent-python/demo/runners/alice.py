...
class AliceAgent(AriesAgent):
    def __init__(
        ...
        **kwargs,
        ...
        self.cred_state = {}
        #start of the full-flow holder timer (set at connect in input_invitation);
    ...
    async def handle_present_proof_v2_0(self, message) -> None:
        #inherit the shared handler unchanged, then time the holder full flow
        #the shared handler does the holder's whole proof step on request-received:
        #query the wallet, build the ZKP, send it. same label and span as the enclave
        #build so the two numbers compare directly
        await super().handle_present_proof_v2_0(message)
        #full-flow holder timer ends at the final ack (done state): print the span from
        #connect (set in input_invitation) to now
...
async def input_invitation(agent_container):
    ...
    #start the full-flow holder timer at connect; it ends at the final ack (done)
