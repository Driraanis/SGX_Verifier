...
from .utils import flatten, log_json, log_msg, log_timer, output_reader, print_ext
...
#marks the agent's combined timing line, and prefixes each span once expanded
NATIVE_SPANS_MARKER = "[timing] native verification: "


def split_native_spans(output: tuple, source: str) -> list:
    """Split a combined native timing line into one line per span.

    Args: output (the relayed line), source (which stream it came from).
    Returns: one string per span, or an empty list if this is not that line.
    """
    if source != "stdout" or len(output) != 1 or not isinstance(output[0], str):
        return []
    if NATIVE_SPANS_MARKER not in output[0]:
        return []
    spans = output[0].partition(NATIVE_SPANS_MARKER)[2].strip().split(" | ")
    return [f"{NATIVE_SPANS_MARKER}{span}" for span in spans]
...
class DemoAgent:
    ...
    def handle_output(self, *output, source: str = None, **kwargs):
        ...
        if source == "stderr":
            ...
            color = None
        #the agent sends its three timing spans as one line because the reader hands the
        #main loop one callback per line and a webhook can run between two of them.
        #expanding here keeps them inside this one callback: print_ext writes straight
        #away, while log_msg would schedule three turns that can interleave again
        spans = split_native_spans(output, source)
        if spans:
            for span in spans:
                #violet like the rest of our own output; the colour goes through
                #prompt_toolkit here, so it renders instead of leaking escape codes
                print_ext(span, color=VIOLET_STYLE, prefix=self.prefix_str, end="\n")
            return
        try:
